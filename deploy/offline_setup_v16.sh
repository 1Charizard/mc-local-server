#!/usr/bin/env bash
# MC1life - 离线安装脚本 v16 (全程无需网络, BDS + 后台 Agent 一体安装)
# v16 变更 (相对 v15):
#   - 去掉 set -e (改 set -uo pipefail + 显式 fail): 任何单步失败不再静默退出
#     (v13-v15 静默退出根因: pipefail + sed '\x1b' 不兼容 -> 管道非零 -> set -e 退出)
#   - 容器检测简化: proot-distro list | grep | awk '{print $NF}' (无需 sed 清 ANSI,
#     容器名是最后一列, ANSI 码在行首不影响)
#   - fallback: list 提取为空时硬编码候选 debian-trixie-aarch64
#   - 冒烟/login 用 timeout 包裹, 防止 proot login 挂起
#   - 内部安装用 if/else 显式判断成功失败
# 用法: bash offline_setup_v16.sh  (与离线文件同目录)
set -uo pipefail

MC1LIFE_SCRIPT_VERSION="v16"
...
log "proot-distro 版本: $(proot-distro --version 2>&1 | head -1 || echo '(无版本输出, 正常)')"

# ---- 1. 容器检测: list 提取容器名 (最后一列, 无需 sed) ----
DISTRO="$(proot-distro list 2>/dev/null | grep -iE 'debian' | awk '{print $NF}' | head -1)"
if [ -n "${DISTRO}" ]; then
  log "检测到容器 (proot-distro list): ${DISTRO}"
else
  # fallback: 硬编码候选 (rootfs 来自 debian-trixie-aarch64.tar.xz)
...
if timeout 900 proot-distro login --bind "${SCRIPT_DIR}:/opt/pack" "${DISTRO}" bash -s <<'INNER'
set -e
