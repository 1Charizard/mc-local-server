#!/usr/bin/env bash
# MC1life - 离线安装脚本 v19 (全程无需网络, BDS + 后台 Agent 一体安装)
# v19 变更 (相对 v18):
#   - 修复 Node 解压失败: 基础 rootfs 无 xz 命令, 无法解 .tar.xz
#     -> Node 包改为 node-*-linux-arm64.tar.gz (gzip 格式, tar 内置支持, 无需外部命令)
#   - NODE_TAR 检测/解压改用 .tar.gz + tar xzf
# 用法: bash offline_setup_v19.sh  (与离线文件同目录)
set -uo pipefail

MC1LIFE_SCRIPT_VERSION="v19"
