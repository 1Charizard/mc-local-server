#!/usr/bin/env bash
# MC1life - 离线安装脚本 v18 (全程无需网络, BDS + 后台 Agent 一体安装)
# v18 变更 (相对 v17):
#   - 修复核心语法 bug: proot-distro 5.8.0 login 必须用 '--' 分隔符
#     `login [OPTIONS] CONTAINER -- COMMAND` (v12-v17 漏了 '--', 导致 -c 被当选项)
#     用户手动验证过的命令: proot-distro login debian-trixie-aarch64 -- /bin/bash -c '...'
#   - 内部安装改为: 生成 install_inner.sh 落盘到 SCRIPT_DIR, login -- 执行它
#     (避免 heredoc stdin 经 proot-distro 转发的不可靠性)
# 用法: bash offline_setup_v18.sh  (与离线文件同目录)
set -uo pipefail

MC1LIFE_SCRIPT_VERSION="v18"
...
# ---- 3. 内部安装 (全程离线) ----
# 生成内部安装脚本 (落盘, 避免 stdin 经 proot-distro 转发不可靠)
