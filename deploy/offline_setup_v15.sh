#!/usr/bin/env bash
# MC1life - 离线安装脚本 v15 (全程无需网络, BDS + 后台 Agent 一体安装)
# v15 变更 (相对 v14):
#   - 修复容器名提取 bug: v13/v14 用 awk '{print $1}' 取到 list 输出的第一列 "*" (星号),
#     而非容器名 -> 改为清理 ANSI 码后取最后一列 ($NF)
#   - 容器检测以 "proot-distro login 实测" 为唯一标准 (最可靠, 与手动验证一致):
#     候选名 = list 提取 + 默认名, 逐个 login true 测试, 命中即用
#   - 保留环境诊断段
# 用法: bash offline_setup_v15.sh  (与离线文件同目录)
set -euo pipefail

MC1LIFE_SCRIPT_VERSION="v15"
...
log "--- 环境诊断 (异常就发我) ---"
log "PREFIX=${PREFIX:-<空>}"
log "proot-distro 版本: $(proot-distro --version 2>&1 | head -1 || echo '(无版本输出)')"
log "list 输出:"
proot-distro list 2>&1 | sed 's/^/    /' || true
log "--- 诊断结束 ---"

# ---- 1. 容器检测: login 实测 (唯一标准) ----
# 候选名: ① list 提取(清 ANSI + 取最后一列) ② 常见默认名, 逐个 login true 验证
LIST_NAME="$(proot-distro list 2>/dev/null | grep -iE 'debian' | sed -E 's/\x1b\[[0-9;]*[a-zA-Z]//g' | awk '{print $NF}' | head -1)"
CANDIDATES=""
for c in "${LIST_NAME}" debian-trixie-aarch64 debian-trixie debian; do
  [ -z "${c}" ] && continue
  case " ${CANDIDATES} " in *" ${c} "*) ;; *) CANDIDATES="${CANDIDATES} ${c}";; esac
done
log "容器候选:${CANDIDATES}"

DISTRO=""
for c in ${CANDIDATES}; do
  if proot-distro login "${c}" true >/dev/null 2>&1; then
    DISTRO="${c}"
...
if [ -z "${DISTRO}" ]; then
  ...
  DISTRO="$(proot-distro list 2>/dev/null | grep -iE 'debian' | sed -E 's/\x1b\[[0-9;]*[a-zA-Z]//g' | awk '{print $NF}' | head -1)"
