#!/usr/bin/env bash
# MC1life - 离线安装脚本 v12 (全程无需网络, BDS + 后台 Agent 一体安装)
# v12 变更 (相对 v10):
#   1. 修复 --bind 位置: proot-distro login 选项必须放在容器名之前
#   2. 修复冒烟测试字符串展开 (单引号, 变量在容器内求值)
#   3. BDS 需要 x86_64 glibc: debs/ 离线安装 amd64 库 (libc6/libstdc++6/libcurl4t64/libssl3t64/...)
#      box64 自带 x86libs bundle 不含 libc.so.6/ld-linux, 无法单独支撑 BDS
#   4. debs/ 同时含 arm64 unzip (基础 rootfs 无 unzip/python3)
#   5. BDS 安装到 /opt/mc1life/bds (与 Agent config.json 默认路径一致)
#   6. 新增 Node.js (linux-arm64 离线包) + Agent (自带 node_modules) 安装,
#      Agent 在容器内运行, 出站 WS 连 Cloudflare Worker (无需内网穿透)
#   7. 幂等: 容器/BDS/Agent 已装则跳过
# 用法: bash offline_setup_v12.sh  (与离线文件同目录)
set -euo pipefail

MC1LIFE_SCRIPT_VERSION="v12"
...
if proot-distro list 2>/dev/null | grep -qiE 'debian'; then
  log "Debian 容器已存在, 跳过安装"
  DISTRO="$(proot-distro list | grep -iE 'debian' | awk '{print $1}' | head -1)"
else
  log "proot-distro install 本地 rootfs (绕开网络)..."
