#!/usr/bin/env bash
# MC1life - 离线安装脚本 v21 (完整包: 全新安装 或 旧版 Agent 自动升级到 v3)
# v21 变更 (相对 v20):
#   - Agent 升级到 v3 (HTTP 轮询架构: 心跳/拉指令/回传结果, 绕开 WS 代理不可靠问题)
#   - 检测逻辑改为 v3 特征 (ws.js 含 PollClient), 旧 v2 版会被自动升级
#   - 配置合并: 强制 workerUrl 用 pages.dev (旧配置 workers.dev 手机连不上)
# 用法: bash offline_setup.sh  (与离线文件同目录)
set -uo pipefail

MC1LIFE_SCRIPT_VERSION="v21"
...
if timeout 60 proot-distro login "${DISTRO}" -- /bin/bash -c 'test -x /opt/mc1life/bds/run.sh && test -x /opt/mc1life/agent/src/index.js && echo ALL_READY || echo INCOMPLETE' 2>/dev/null | grep -q ALL_READY; then
  if timeout 30 proot-distro login "${DISTRO}" -- /bin/bash -c 'grep -q "PollClient" /opt/mc1life/agent/src/ws.js && echo V3 || echo OLD' 2>/dev/null | grep -q V3; then
    log "检测到 BDS + 新版 Agent (v3 轮询) 已安装, 跳过安装"
    show_usage "${DISTRO}"
    exit 0
  else
    log "检测到已安装但 Agent 不是 v3 (缺轮询), 仅更新 Agent..."
    AGENT_ONLY=1
  fi
fi
...
if [ "${AGENT_ONLY}" = "1" ]; then
  ...
  # 合并旧 config.json: 保留凭证 (token/r2), 但强制 workerUrl 用 pages.dev (旧 workers.dev 手机连不上)
  if [ -f "${BK}/config.json" ]; then
    log "合并旧配置 (保留凭证, 强制 pages.dev 地址)..."
...
// 关键: workerUrl 必须用 fresh 的 pages.dev (旧配置可能是 workers.dev, 手机不可达)
merged.workerUrl = fresh.workerUrl;
fs.writeFileSync('/opt/mc1life/agent/config.json', JSON.stringify(merged, null, 2));
console.log('config.json 合并完成, workerUrl:', merged.workerUrl);
NODEEOF
  else
...
# --- Agent (v3 完整包, 含真实凭证 + pages.dev 地址) ---
...
if [ "${AGENT_ONLY}" = "1" ]; then
  echo
  log "════════════════════════════════════════════════════"
  log " ✅ Agent 已升级到 v3 (HTTP 轮询, 面板在线管理)"
  log ""
