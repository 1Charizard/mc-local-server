#!/usr/bin/env bash
# MC1life - 离线安装脚本 v17 (全程无需网络, BDS + 后台 Agent 一体安装)
# v17 变更 (相对 v16):
#   - 修复根因: Termux 的 proot-distro list 输出在 STDERR, v16 用 2>/dev/null 丢弃
#     -> 提取为空 -> 误判未安装 -> 重复 install -> "already exists"
#   - 容器检测顺序: ① login 实测 debian-trixie-aarch64 (用户已验证, 最可靠)
#     ② list 2>&1 + grep -o 提取纯容器名 (不依赖 awk/sed 转义)
#     ③ 都没有才 install
# 用法: bash offline_setup_v17.sh  (与离线文件同目录)
set -uo pipefail

MC1LIFE_SCRIPT_VERSION="v17"
...
# 方式 ②: list 解析 (2>&1 保留 stderr + grep -o 纯提取容器名)
if [ -z "${DISTRO}" ]; then
