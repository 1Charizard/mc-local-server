#!/usr/bin/env bash
# MC1life - 离线安装脚本 v14 (全程无需网络, BDS + 后台 Agent 一体安装)
# v14 变更 (相对 v13):
#   - 容器检测升级为三路: ① proot-distro list ② 安装目录 ③ login 实测候选名 (最可靠)
#   - 内置诊断输出 (PREFIX/proot-distro 路径/list 输出/容器目录), 便于定位环境差异
# 用法: bash offline_setup_v14.sh  (与离线文件同目录)
set -euo pipefail

MC1LIFE_SCRIPT_VERSION="v14"
...
# 方式 C: login 实测候选名 (最可靠, 与用户手动验证一致)
if [ -z "${DISTRO}" ]; then
